...
func AddCommentRoutes(r *gin.Engine, db database.IDbConn, p *bluemonday.Policy, server ws.IServer) {
	rep := comment_postgres.NewRepo(db)
	uc := comment_usecase.NewUsecase(rep)
	handler := NewHandler(uc, p)

	r.POST("/pin/comments", auth.AuthCheck(), middleware.SendNotification(note_http.CreateNoteUsecase(db, server)), handler.CreateComment) // Use(csrf.CSRFCheck()) на фронте его еще нет, поэтому закомменчен
